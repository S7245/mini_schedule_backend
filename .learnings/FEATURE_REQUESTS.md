...
- 下一主线：Booking/Waitlist/Attendance/Entitlement（学员预约批次）。
- 场次改期 reschedule；循环排课整批取消（blueprint v1 不做，记备 v2）。
- per-brand 时区（当前固定 Asia/Shanghai +08:00）。
- exclusionConstraint 空约束名退化歧义（沿 12a）。

## 2026-06-22 Batch 13c
- 23514 CHECK 违反兜底分流（容量/余额，镜像 isUniqueViolation 23505 路径）。
- 频次限额严格并发（profile advisory lock）——当前 TOCTOU 软策略可轻微突破。
- 员工代取消绕过 cancel_deadline（当前对所有来源一律生效）。
- 预约窗口/取消截止的 per-brand 时区（频次日/周/月界当前用 UTC，合并 12b per-brand TZ FR）。
- 场次取消级联无分页（大场次一次 Find 全部 booked 逐条处理，长事务，性能项）。
- 学员同一时段多场次冲突检查（§22.1 失败场景「时间冲突」，13c 仅 unique 防同场次重复）。
- booking 域 session.* vs schedule.* 权限码：classsession service require "session.view" 但 000003 seed 的是 schedule.*，需核对是否有 session.* migration 或统一（booking 已用确认 seed 的 booking.*/schedule.*）。

## 2026-06-22 Batch 13d
- 候补转正微信订阅通知（§22.4「发送候补转正消息」，brand 后台无微信，留 C 端 14/通知批）。
- 自动转正（booking 取消即顺位转正/标 eligible），需先解决无权益自动处理策略。
- 候补位置手动调整/插队。
- 满员场次受 session list page_size=100 限制（既有分页，非本批引入）；超 100 名外的满员场次在代预约下拉选不到 → 候补入口也受限。
- 候补随场次过期/完成的清理（当前仅场次取消级联 cancel 候补）。

## 2026-06-24 Batch 13e
- 爽约退课响应直出 released hold 产品名（须改 baseQuery DISTINCT ON booking_id ORDER BY id DESC 并回归 13c 取消列表）。
- 撤销误签到（attended→撤销，§20.12 第一版走人工权益调整；可加专用反向事务）。
- 批量一键签到（场次全员到课）；session_records 富履约数据（评价/训练量，metadata 列已留）。

## 2026-06-25 Batch 14a
- 真实 WeChat code2session（per-brand 小程序 AppID/secret）替换 dev 占位 openid "dev_"+code。
- 手机号绑定/授权（§7.1）：v1 profile 可无手机号；绑定后回填 learner_identities.phone。
- 同人 by-phone 档 ↔ by-openid 登录 identity 合并（v1 是两个 profile）。
- legacy app_users 退役（C 端双轨：app_user + learner profile 并存）。
- 并发首登 identity/profile INSERT 撞唯一约束后回查在已 abort 的 tx 内会失败（13a 既有模式，登录重试自愈）；要真正幂等首登把 insert-then-recover 包 savepoint(nested tx)。
- C 端候补转正通知 / 真实订阅消息（§7.5）。

## 2026-06-26 Batch 14b（code-review P2，转 FR）
- `CancelByLearner` 非活跃态复用 `WAITLIST_NOT_PROMOTABLE` 错误码（语义偏「转正」非「取消」）——与 staff transition 一致，C 端若按 code 分支再细分。
- `ListMyBookings` 的 status 逗号 split 无 allowlist（无注入风险、brand+profile scoped、未知状态空集）；如需严格可校验 ∈ 合法状态集。
- 候补转正(staff-manual)、转正/上课提醒微信订阅消息仍留 FR（§7.5）。

## 2026-06-26 Batch 15（场次状态自动化，转 FR）
- **`ListDueSessionIDs` 加 `(status, ends_at)` partial 覆盖索引**（code-review F3）：扫描 `status IN (scheduled,in_progress) AND ends_at<=now` 每分钟跑、跨品牌、无覆盖索引（现有索引 lead on brand_id 或 instructor）。v1 规模 seqscan 可忽略（稳态命中集极小、completed/cancelled 不在谓词内），但 class_sessions 历史行增长后建 `CREATE INDEX ... ON class_sessions(ends_at) WHERE status IN ('scheduled','in_progress')`（需 migration 000013，本批零 migration 决策故 defer）。
- **复用 worker 框架做 subscription 自动 grace/expired**（设计文档 §11）：BrandSubscription 自动进 GracePeriod/Restricted/Expired 是设计文档列明的定时任务缺口；现 asynq worker 框架已就位，加一个 periodic task + 用例服务即可，模式同场次 sweep。
- **`asynq.Unique(ttl)` 防重叠 enqueue**（可选优化）：当前靠幂等兜底（安全），如 sweep 在高负载下耗时 >cron 间隔会堆积无谓任务；加 Unique 可省。TTL 须与 cron 间隔耦合，留待 sweep_cron 可配后再定。
- **per-brand 时区调度**：sweep 用 UTC 即时比较（正确），但未来若要「按品牌营业时区在特定钟点跑某些自动化」，需引入 per-brand TZ（合并 12b/13b/13c 既有 TZ FR）。

## 2026-06-27 Batch 16（订阅生命周期自动化，code-review high 转 FR）
- **手动 `UpdateBrandSubscriptionStatus→grace_period` 留 stale/NULL grace_ends_at**（code-review 最有价值发现，已核实可达：admin allowlist 接受 grace_period，repo 仅在 `→active` 清 grace_ends_at）。放宽 guard 后果：手动置 grace_period 且 grace_ends_at=NULL 的 sub 被 guard 视同可用(走 expires_at 时间门)、且 `ListSubscriptionsDueForRestricted` 要求 `grace_ends_at IS NOT NULL`→**永不被自动 restrict**（「宽限不结束」，仅 expires_at 兜底）。低危(manual 旁路、后果被 expires_at 界定、触发非典型)留 FR。**修向**：手动转 grace_period 时按 grace_days 补算 grace_ends_at，或 guard grace 分支要求 grace_ends_at 非空。**警告**：未来做 restricted→expired 自动化若按 `status='restricted' AND grace_ends_at<=now` 扫，手动转 restricted 留的 stale grace_ends_at 会被立刻误扫 expired——届时须先把 manual 路径的 grace_ends_at 语义理清。
- **自动 `restricted→expired`**（§1389 列 Expired 态，§1335 只到 Restricted）：触发语义未定（restricted 多久后 expired？）留 FR。本批 restricted=终态。
- **§4.5 受限态对「新预约/新场次/新权益」的订阅门**：现 booking/classsession/entitlement repo 不查订阅 status，本批 restricted 只拦 Location/Staff/Learner 供给（SubscriptionGuard）。补全需在 3-5 写入点加门 + 跨 13c–14b 回归，pre-existing 执行面缺口，独立批/FR。
- **支付异常人工补偿开通订阅**（PROGRESS §3 P0）：独立 admin 流程，非自动化。
- **restricted 品牌再购撞 `idx_brand_subscriptions_one_current`**：再购前须平台手动 cancel/expire 旧 sub，pre-existing 边界。
- **brand 后台受限/续费 banner + grace_period 在 admin dashboard summary 单独计数**（现 grace 不入 active/restricted/frozen 任何桶，自动化产 grace 后 active 计数会少掉宽限中品牌→运营可能误读为流失）。
- **「可用/有效订阅 status 集」集中化**（code-review altitude）：现 guard(`IN ('active','grace_period')`)与 dashboard summary(`status='active'`)各为独立 SQL 字面量，未来加态(如 trial)易漂移；宜抽 domain helper/常量集。
- **到期前提醒通知**（§4.5 到期前提醒=站内/微信订阅消息）：留 Batch 18 通知中心。

## 2026-06-27 Batch 17（品牌基础运营看板，留 FR）
- **17b 平台看板**：§15 平台看板 9 指标，admin GetPlatformSummary 已覆盖 ~5（品牌总数/活跃/付费/即将到期/今日订单），缺 **套餐分布 + Location/员工席位/学员用量 + 本月订单收入**。独立批补全 admin 端。
- **报表导出中心 / §15「第一版不做」**：收入分析、支付对账、退款、教练绩效薪酬、课程转化率、学员留存、渠道来源、LTV、经营驾驶舱、导出。
- **per-brand 时区报表窗口**：现窗口边界 UTC（resolveWindow），per-brand 营业时区留既有 TZ FR（合并 12b/13b/13c）。
- **爽约数精确确认时间锚点**：bookings 无 no_show_at 列，现 A 组爽约数锚定父场次 starts_at 代理；若要"本周确认了多少爽约"须从 session_records/entitlement_consumptions(no_show) 取确认时间。
- **取消数按 cancel_source 拆分**：现取消数含所有 status='cancelled'（含 session_cancelled 级联）；可按 learner/staff/session_cancelled 拆。
- **Instructor「自己场次数据」报表**（21.2）：instructor 未 seed report.view_basic，未来开放教练看自己场次。
