...
## 2026-06-18 Batch 13b — 权益产品 + 发放（EntitlementProduct/LearnerEntitlement/Transaction）

### 复用 000003 粗权限码 + 只补缺失映射（不是每批都加新码）
000003 已 seed `entitlement.view/manage/adjust` 三码且语义恰好对上 §20.9（view/manage/adjust）+ §21.1，只是模板映射不全。13b migration 000011 **不加新码、不动表**，只补 3 条缺失映射（brand_admin.adjust/course_operator.view/finance_support.adjust）+ backfill（VALUES CTE join brand_roles，精确按 (template,code) 对）。规则：新域权限先 grep 000003 看有没有合用的粗码——粗码语义对得上就复用+补映射，别无脑拆细码（对比 13a learner 要细码是因为 §21 owner/admin 写、其余只读，三分法 view/manage/adjust 不够）。

### 派生态「落库」无 cron：纯函数 + 读触发 sweep + 写后落库
expired/depleted 要落库但无定时任务：①`SettleStatus(current,expires,total,remaining,now)` 纯函数（frozen/cancelled 透传；expires<=now→expired；total!=nil&&remaining<=0→depleted；否则 active）放 domain 可单测；②读时 `settleSweepLearner` 一条 per-learner 批量 UPDATE（`WHERE status='active' AND (expires<=now OR remaining<=0)`，CASE 与纯函数同逻辑）；③grant/adjust/status 变更后对该行 settle 落库。未访问的过期权益首次读到才落库——V1 可接受。SQL CASE 必须和 Go 纯函数逐条对齐（含 expiry 优先于 depleted）。

### lock→consume 模型分批落地：先写一半，列预留
13b 只做 grant（快照 total→remaining，membership total NULL=不限次）+ manual adjust（remaining±delta）；`locked_credits`/`consumed_credits` 恒 0，hold(13c)/consume(13e) 才动。adjust 用 `SELECT FOR UPDATE`（clause.Locking）+ 非负校验→`ENTITLEMENT_INSUFFICIENT`（不裸触 CHECK 23514→500），这套并发骨架 13c hold 直接复用。

### scope 关联校验用存在性，不卡 active（避免编辑被存量拖死）
产品 location/course scope 关联（all/specific + 关联表硬删重插，镜像 course_location_availability）校验 ids ⊆ 本 brand **存在性**，不卡 active/published。否则产品建好后门店停用/课程归档，编辑产品（改名）就被 `ENTITLEMENT_SCOPE_INVALID` 强拒（停用项对 13c 选权益只是不匹配，无害）。新增 scope 时前端只给 active 选项，UI 上不会新绑停用项。
...
## 2026-06-27 Batch 17（品牌基础运营看板，纯读聚合）

### 报表聚合 = onboarding GetCounts 范式推广（N 条聚合 SQL 一次出全指标，不分页伪装统计）
新 `report_repository.BrandOverviewCounts` 镜像 `onboarding_repository.GetCounts`：一次 repo 调用跑多条 `COUNT`/`GROUP BY`/`FILTER` 聚合 SQL（CLAUDE.md 铁律：统计走专用聚合，不是 List 出来前端数）。指标 1-4 用 `COUNT(*) FILTER (WHERE status=…)` 一趟出（比逐 query 更省）。GORM `db.Raw(sql,args...).Scan(&slice)` 直接扫多行进 `[]struct`，列名→字段按 snake_case 映射（`ip.display_name AS name`→`Name`、`COUNT(*) AS session_count`→`SessionCount`），无需 gorm tag。

### 报表口径分三组、每指标按各自锚点过滤（严禁单一全局 created_at）
**A 组周期活动**锚定 `class_sessions.starts_at` 落窗口 + `cs.status IN (scheduled,in_progress,completed)`，按 `booking.status` 分类（预约=非取消/到课/取消/爽约）——session-period 框定让 A 组数字互相对得上、且天然处理 no_show 无时间列；**B 组周期事件**锚定事件自身时间戳（holds.held_at / consumptions.consumed_at）；**C 组实时积压**忽略时间窗（pending_no_show / waitlist 活跃态）= "现在待处理多少"。上座率分母只取 `completed` 场次容量（未来已排但未上的不稀释），`if TotalCapacity>0` 防除零。subagent 标注：单一全局 created_at 会把"本周取消的旧预约"算进本周预约——必须逐指标定锚点写进契约。

### data_scope 统一过滤 cs.location_id：location-less 表经 NOT NULL FK join 收紧（不改计数）
所有聚合查询都 join 到 `class_sessions cs`，故 scope 统一过滤 `cs.location_id`。holds/consumptions/waitlist/pending 这些 location-less 表经各自 NOT NULL FK（booking_id/class_session_id）join 到 cs，因 FK NOT NULL 且一对一，join 不丢不增行 → 可**始终 join**、scope 空时不加过滤即可（无需条件 join 两套代码）。scopeFilterIDs 镜像 13c：nil=全品牌、空切片=`AND 1=0` 全 0、单门店 LocationID（service 层 guardLocationInScope 先验 ∈ scope）。IN 用手搓 `(?,?)` 占位（GORM Raw 的 slice 展开不稳，手搓最稳）。winArgs/liveArgs 各建新 `[]any` 字面量避免 append 别名 bug，scope 占位恒在 brand/窗口占位之后。

### 报表权限门在 application service（Batch 6 模式），不在路由中间件
`report.view_basic` 门 = `service.GetBrandOverview` 首行 `checker.Require`（非 route middleware），与 booking/waitlist 一致。handler 极薄（透传 query），JWTAuth 在 auth 组保证认证。Wire 加 repo+service+handler+`wire.Bind(report.PermissionChecker→*rbac.Checker)` 后 `/Users/liushan/go/bin/wire ./cmd/api-brand/` 重生成 wire_gen.go（wire CLI 在 GOPATH/bin，不在 PATH）。改 NewHandler 形参须同步 wire_gen 调用点 + router_test 的 nil 占位数（go build 会抓到不一致）。
