...
## 2026-06-25 Batch 14a — C 端 greenfield 既有阻断（桥接首次端到端跑暴露）

### app_users.vip_level 列 tag 缺失 → 新用户登录 INSERT 42703（既有，已修）
AppUserModel.VIPLevel 无 `gorm:"column:..."` → GORM 派生列名 v_ip_level，实际列是 vip_level → 每次新 app_user INSERT 报 `column "v_ip_level" does not exist (42703)`，C 端新用户登录必失败。SELECT * 不报（按列名回填），仅 INSERT 显式列名现形。修：加 column:vip_level。Pending exposure：凡 GORM model 字段含连续大写缩写（VIP/URL/ID/API）且无显式 column tag，派生名可能与列不符——grep 核对。

### app router 无 CORS → 浏览器跨域被拦（既有，已修）
brand/admin router 都 r.Use(middleware.CORS(cfg.CORS))，唯 app router 只有 Locale+Recovery；config-app.yaml 无 cors 段。C 端浏览器(:3003)/小程序直连后端(:8082)跨域 → 预检无 Access-Control-Allow-Origin（client.ts 对 /api/v1/app/ 不走同源代理、直连 NEXT_PUBLIC_API_URL）。修：app router 加 CORS + config-app.yaml cors.allowed_origins。生产经 MINI_SCHEDULE_CORS_ALLOWED_ORIGINS 注入 Vercel 域名（注意 config.go keysToBind 需含 cors.allowed_origins 才能 env 覆盖，见 CLAUDE.md）。
